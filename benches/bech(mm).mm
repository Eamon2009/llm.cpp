...
static void ensure_dir(const std::string &dir)
{
      ...
      NSString *nsDir = @(dir.c_str());
      [fm createDirectoryAtPath:nsDir withIntermediateDirectories:YES attributes:nil error:nil];
}
...
int main(int argc, char **argv)
{
      @autoreleasepool {
            ...
            for (NSUInteger i = 1; i < arguments.count; ++i)
            {
                  ...
                  if ([arg isEqualToString:@"--data"] && i + 1 < arguments.count)
                        opt.data_path = [arguments[++i] UTF8String];
                  else if ([arg isEqualToString:@"--model"] && i + 1 < arguments.count)
                        opt.model_path = [arguments[++i] UTF8String];
                  else if ([arg isEqualToString:@"--out"] && i + 1 < arguments.count)
                        opt.out_dir = [arguments[++i] UTF8String];
                  else if ([arg isEqualToString:@"--runs"] && i + 1 < arguments.count)
                        opt.runs = [arguments[++i] intValue];
                  else if ([arg isEqualToString:@"--warmup"] && i + 1 < arguments.count)
                        opt.warmup = [arguments[++i] intValue];
                  else if ([arg isEqualToString:@"--batch-size"] && i + 1 < arguments.count)
                        opt.batch_size = [arguments[++i] intValue];
                  else if ([arg isEqualToString:@"--train-steps"] && i + 1 < arguments.count)
                        opt.train_steps = [arguments[++i] intValue];
                  else if ([arg isEqualToString:@"--generate-tokens"] && i + 1 < arguments.count)
                        opt.generate_tokens = [arguments[++i] intValue];
                  else if ([arg isEqualToString:@"--max-data-chars"] && i + 1 < arguments.count)
                        opt.max_data_chars = [arguments[++i] intValue];
                  else if ([arg isEqualToString:@"--random-weights"])
                        opt.random_weights = true;
                  else if ([arg isEqualToString:@"--quick"])
                        opt.quick = true;
            }
