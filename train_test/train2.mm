...
int main(void)
{
      ...
      @autoreleasepool
      {
            ...
            NSString *vocabPath = [@"../vocab.bin" stringByStandardizingPath];
            NSString *weightsPath = [@"../weights.bin" stringByStandardizingPath];
            ...
            FILE *fv = fopen([vocabPath UTF8String], "rb");
            if (!fv)
            {
                  fprintf(stderr, "[Error] Cannot open vocab.bin\n");
                  return 1;
            ...
            if (![fm fileExistsAtPath:weightsPath])
            {
            ...
            FILE *fw = fopen([weightsPath UTF8String], "rb");
            if (!fw)
            {
                  fprintf(stderr, "[Error] Cannot open weights.bin\n");
                  free(vocab);
